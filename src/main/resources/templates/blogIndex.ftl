...
<div class="container custome-container">
            ...
            <div class="scrolltext">
                <ul class="list-unstyled" id="notice-box">
                    <li class="scrolltext-title">
                        <a href="javascript:void(0)" rel="bookmark">本站正式启用新域名:<a href="http://www.cchcz.com"
                                                                                 target="_blank"><strong>http://www.cchcz.com</strong></a></a>
                    </li>
                    <li class="scrolltext-title">
    ...
    <div class="row">
        <div class="col-sm-8 blog-main">
            <div class="blog-body expansion" style="padding: 0;">
                <#if carouselArticles?? && carouselArticles?size gt 0>
                    <div id="myCarousel" class="carousel slide" style="height:300px;">
                        <ol class="carousel-indicators">
                            <#list carouselArticles as carouselArticle>
                                <li data-target="#myCarousel" data-slide-to="${carouselArticle_index}"
                                    <#if carouselArticle_index==0>class="active"</#if>></li>
                            </#list>
                        </ol>
                        <div class="carousel-inner">
                            <#list carouselArticles as carouselArticle>
                                <div <#if carouselArticle_index==0>class="item active" <#else>class="item" </#if>>
                                    <a href="${config.siteUrl}/article/${carouselArticle.id?c}">
                                        <img src="${config.staticWebSite}${carouselArticle.coverImage}"
                                             alt="${carouselArticle.title}" title="${carouselArticle.title}">
                                    </a>
                                    <div class="blogweb-carousel-caption">${carouselArticle.title}</div>
                                </div>
                            </#list>
                        </div>
                            ...
                            <a href="${config.siteUrl}/article/${item.id?c}">
                                <#if item.coverImage?exists && (item.coverImage?length > 7)>
                                    <img width="150" height="150"
                                         data-original="${config.qiuniuBasePath}${item.coverImage}"
                                         class="img-responsive lazy-img" alt="${item.title?if_exists}">
                                <#else>
                                    <img width="150" height="150"
                                         data-original="${config.staticWebSite}/images/default_article_cover.jpg"
                                         class="img-responsive lazy-img" alt="${item.title?if_exists}">
                                </#if>
                            </a>
                            <span class="cat"><a
                        ...
                        <div class="entry-content">
                            ...
                            <span class="entry-meta">
                                <span class="date" title="文章发表日期" data-toggle="tooltip" data-placement="bottom"><i
                                        class="fa fa-calendar-o fa-fw"></i>${item.createTime?string('yyyy-MM-dd')}</span>
                                <span class="views" title="文章阅读次数" data-toggle="tooltip" data-placement="bottom"><i
                ...
                <article class="fade-in" style="height: auto">
                    <div class="rows">
                        ...
                        <div class="col-md-2 col-sm-12">
                            <img src="${config.staticWebSite}/images/wrong.gif" class="center-block" alt="惩罚我"
    ...
    <#--按分类推荐-->
        <div class="col-xs-12 col-sm-12 col-md-12 blogweb-container">
            <div class="blogweb-ad" data-position="index_1" style="margin-bottom: 10px;max-height: 135px;"></div>
        </div>
        <div class="col-xs-12 col-sm-6 col-md-6 blogweb-container">
            <div class="blogweb-box">
                <div class="category">
                    <div style="color: #e04620;"><i class="fa fa-fire fa-fw fa-2x"></i>热门文章</div>
                </div>
                <div class="blogweb-content">
                    <div class="row">
                        <div class="col-md-12 col-xs-12 overflow-initial">
                            <ul class="list-unstyled alist">
                    <@articleTag method="hotList" pageSize="6" >
                        <#if hotList?exists && (hotList?size > 0)>
                            <#list hotList as item>
                                <#if item_index==0>
                            <li class="item-date">${item.createTime?string('MM-dd')}</li>
                                <li class="item-title item-title-0">
                                    <a class="t" href="${config.siteUrl}/article/${item.id?c}"
                                       data-original-title="${item.lookCount!(0)}人浏览了该文章"
                                       data-toggle="tooltip" data-placement="bottom">
                                        ${item.title!''}
                                    </a>
                                    <br>
                                    <div class="item-head">
                                        <figure class="blogweb-thumbnail" style="float: left;margin-right: 20px;">
                                            <a href="${config.siteUrl}/article/${item.id?c}">
                                                <img data-original="${config.staticWebSite}${item.coverImage}"
                                                     class="img-responsive lazy-img" alt="热门文章"
                                                     style="float: left;width: 150px;height: 120px;">
                                            </a>
                                        </figure>
                                        <div class="item-head-desc">${item.description!''}
                                        </div>
                                    </div>
                                </li>
                                <#else>
                             <li class="item-date">${item.createTime?string('MM-dd')}</li>
                                <li class="item-title item-title-1">
                                    <a class="t" href="${config.siteUrl}/article/${item.id?c}"
                                       data-original-title="${item.lookCount!(0)}人浏览了该文章"
                                       data-toggle="tooltip" data-placement="bottom">
                                        ${item.title!''}
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 blogweb-container">
            <div class="blogweb-box">
                <div class="category">
                    <div><i class="${subType.icon!''} fa-fw fa-2x"></i>${subType.name}</div>
                </div>
                <div class="blogweb-content">
                    <div class="row">
                        <div class="col-md-12 col-xs-12 overflow-initial">
    ...
    <ul class="list-unstyled list-inline">
        <#if indexLinkList?exists && (indexLinkList?size > 0)>
            <#list indexLinkList as item>
                <li>
                    <a href="${item.url}" target="_blank" title="${item.description?if_exists}" data-toggle="tooltip"
                       data-placement="bottom">
                        <img src="${item.favicon}" alt="${item.name?if_exists}"
                             onerror="this.src='${config.staticWebSite}/images/user.png'">${item.name?if_exists}
                    </a>
                </li>
            </#list>
            <li>
                <a href="${config.siteUrl}/links" title="更多链接" data-toggle="tooltip" data-placement="bottom"><i
