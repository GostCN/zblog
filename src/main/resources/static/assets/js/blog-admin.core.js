...
var blogadmincore = window.blogadmincore || {
    ...
    initValidator: function () {
        "undefined" != typeof validator && (console.log("blogadmincore.initValidator"), validator.message.date = "not a real date", $("form").on("blur", "input[required], input.optional, select.required", validator.checkField).on("change", "select.required", validator.checkField).on("keypress", "input[required][pattern]", validator.keypress), $(".multi.required").on("keyup blur", "input", function () {
            ...
            editor.customConfig.uploadImgHooks = {
                ...
                customInsert: function (insertImg, result, editor) {
                    ...
                    if (result.status == 200) {
                        ...
                        var imgFullPath = appConfig.qiniuPath + result.data;
                        ...
                        editor.txt.append('<img src="' + imgFullPath + '" alt="" style="max-width: 100%"/>');
                    } else {
...
$.fn.popover.Constructor.prototype.leave = function (a) {
    ...
    blogadmincore.initDaterangepicker();
    blogadmincore.initValidator();
    blogadmincore.initSidebar();
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (!json.data) {
                var html = '<li><a><span class="image"><img src="' + appConfig.staticPath + '/assets/images/loading.gif" alt="user avatar"></span> <span><span>系统管理员</span> <span class="time">3 mins ago</span></span> <span class="message">暂无消息</span></a></li>';
                $box.before(html);
                return;
            }
            var tpl = '{{#data}}<li><a href="/admin/comments"><span class="image"><img src="{{avatar}}" alt="user avatar"></span> <span><span>{{nickname}}</span> <span class="time">{{createTimeString}}</span></span> <span class="message">点击查看&审核</span></a></li>{{/data}}';
    ...
    $("img.lazy-img").lazyload({
        placeholder: appConfig.staticPath + "/images/loading.gif",
        effect: "fadeIn",
        threshold: 100
    });
