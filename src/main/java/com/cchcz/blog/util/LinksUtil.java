...
import com.cchcz.blog.constant.HttpConstant;
...
public class LinksUtil {
    ...
    public static String getFavicon(String url) {
        if (!url.startsWith(HttpConstant.HTTP_PROROCOL) && !url.startsWith(HttpConstant.HTTPS_PROROCOL)) {
            url = HttpConstant.HTTP_PROROCOL + url;
        }
    ...
    public static boolean hasLinkByChinaz(String url, String domainName) {
        if (url.startsWith(HttpConstant.HTTP_PROROCOL) || url.startsWith(HttpConstant.HTTPS_PROROCOL)) {
            url = url.replace(HttpConstant.HTTP_PROROCOL, "");
            url = url.replace(HttpConstant.HTTPS_PROROCOL, "");
        }
        try {
            ...
            for (int i = 1; i < liElements.size(); i++) {
                ...
                if (text.startsWith(domainName) || text.startsWith("www." + domainName)
                        || text.startsWith(HttpConstant.HTTP_PROROCOL + domainName) || text.startsWith(HttpConstant.HTTP_PROROCOL + "www." + domainName)
                        || text.startsWith(HttpConstant.HTTPS_PROROCOL + domainName) || text.startsWith(HttpConstant.HTTPS_PROROCOL + "www." + domainName)) {
                    return true;
                }
            }
        } catch (Exception e) {
