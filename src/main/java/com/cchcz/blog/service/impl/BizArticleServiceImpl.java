...
import com.cchcz.blog.dao.mapper.BizArticleLookMapper;
import com.cchcz.blog.dao.mapper.BizArticleLoveMapper;
import com.cchcz.blog.dao.mapper.BizArticleMapper;
import com.cchcz.blog.dao.mapper.BizArticleTagsMapper;
...
import com.cchcz.blog.service.BizArticleTagsService;
...
public class BizArticleServiceImpl implements BizArticleService {
    ...
    public void doPraise(Long id) {
        String ip = IpUtil.getRealIp(RequestUtil.getRequest());
        String key = ip + "_doPraise_" + id;
        ValueOperations<String, Object> operations = redisTemplate.opsForValue();
        if (redisTemplate.hasKey(key)) {
            throw new BlogArticleException("一个小时只能点赞一次哈，感谢支持~~");
        }
        ...
        love.setUserIp(IpUtil.getRealIp(RequestUtil.getRequest()));
    ...
    public boolean publish(Article article, Long[] tags, MultipartFile file) {
        ...
        if ((articleId = article.getId()) != null) {
            ...
            articleContent.setContentMd(article.getContentMd());
            bizArticleContentService.updateSelective(articleContent);
        } else {
            ...
            articleContent.setContent(article.getContent());
            articleContent.setContentMd(article.getContentMd());
            bizArticleContentService.insert(articleContent);
        }
        if (articleId != null) {
    ...
    @Override
    public List<Article> queryByIds(List<Long> ids) {
        List<BizArticle> bizArticles = bizArticleMapper.queryByIds(ids);
        if (CollectionUtils.isEmpty(bizArticles)) {
            return null;
        }
        Map<Long, BizArticle> collect = bizArticles.stream().collect(Collectors.toMap(s -> s.getId(), s -> s));
        return ids.stream()
                .map(id -> collect.get(id))
                .map(b -> new Article(b))
    ...
    public boolean removeByPrimaryKey(Long primaryKey) {
        ...
        bizArticleContentService.removeByPrimaryKey(primaryKey);
        // 删除标签记录
    ...
    public Article getByPrimaryKey(Long primaryKey) {
        ...
        ArticleContent articleContent = bizArticleContentService.getByPrimaryKey(primaryKey);
        if (entity != null && articleContent != null) {
            entity.setContent(articleContent.getContent());
            entity.setContentMd(articleContent.getContentMd());
        }
